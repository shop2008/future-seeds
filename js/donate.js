document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateForm() {
    ...
    isValid =
      validateTextField(firstName, 'Please enter your first name.') && isValid;
    isValid =
      validateTextField(lastName, 'Please enter your last name.') && isValid;
    ...
    const cvvRegex = /^\d{3}$/;
    isValid = validateFieldWithRegex(cvv, cvvRegex) && isValid;
  ...
  function showConfirmationModal(amount) {
    createDynamicModal({
      ...
      onConfirm: function () {
        sendDonation(amount); // Send data to database
      },
      cancelText: 'Cancel',
      onCancel: function () {
  ...
  function sendDonation(amount) {
    ...
    fetch(
      'https://xu19b3ejb3.execute-api.us-east-2.amazonaws.com/future-seeds',
      {
        method: 'POST',
        headers: {
        ...
        // Send the payload directly, no need to wrap it in another object
      ...
      .then((data) => {
        ...
        if (data.statusCode === 200) {
          replaceContentWithSuccessMessage(amount); // Replace content if the request is successful
        } else {
          console.error('Error in Lambda response:', data);
          alert(
